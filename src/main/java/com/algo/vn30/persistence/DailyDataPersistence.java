...
public interface DailyDataPersistence extends CrudRepository<DailyDataImpl, Long> {
    ...
    @Query("SELECT t FROM DailyDataImpl t WHERE t.sec_id = :sec_id and t.date <= :date order by t.date desc")
    List<DailyDataImpl> findBySecIdAndDateBefore(@Param("sec_id") Long sec_id, @Param("date") Date date);
